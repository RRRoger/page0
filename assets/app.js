...
function showDetail(n,fromMap=false){selected=n;const d=DATA.find(x=>x.n===n);$('#list').style.display='none';$('#detail').style.display='block';$('#sheetTitle').textContent=`点位 ${String(n).padStart(2,'0')} / 20`;const fields=['编号','商圈','楼盘性质','交房时间','收盘售价','户数','车位数','环线描述','定位点','距离（米）'];const query=encodeURIComponent('上海市'+d['区域']+'区'+d['地址']+' '+d['楼盘名称']);$('#detail').innerHTML=`<div class="detail-head"><div class="id">${d['区域']}区 · ${d['商圈']}</div><h3>${escapeHTML(d['楼盘名称'])}</h3><p class="address">上海市${d['区域']}区 · ${escapeHTML(d['地址'])}</p><div class="tags"><span class="tag">${d['形式']}</span><span class="tag">${d['环线描述']}</span></div></div><div class="key-info"><div><small>道闸位置</small><strong>${escapeHTML(d['道闸位置'])}</strong></div><div><small>门的方位</small><strong>${escapeHTML(d['门的方位'])}</strong></div></div><div class="checkin-panel"><button id="checkin" class="checkin-button ${isChecked(n)?'is-checked':''}" aria-pressed="${isChecked(n)}">${isChecked(n)?'已签到 · 取消签到':'签到'}</button><p>${isChecked(n)?'签到时间：'+escapeHTML(new Date(checkins[d['编号']]).toLocaleString('zh-CN')):'签到后地图标记变为蓝色'}<br>仅保存在当前浏览器</p></div><div class="detail-actions"><a class="primary" href="https://api.map.baidu.com/uri/v1/search?query=${query}&region=上海&output=html&src=ningbo_bank_points" target="_blank" rel="noopener noreferrer">在百度地图查看 <span aria-hidden="true">↗</span></a><button class="secondary" id="copyAddress">复制地址</button></div><p class="note">${escapeHTML(d.positionNote)}。请以原表门位描述及现场为准。<a href="${d.coordSource}" target="_blank" rel="noopener noreferrer">位置来源</a></p><dl>${fields.map(k=>`<dt>${k==='距离（米）'?'距定位点':k}</dt><dd>${escapeHTML(typeof d[k]==='number'&&k!=='交房时间'?d[k].toLocaleString('zh-CN'):d[k])}${k==='距离（米）'?' 米':k==='户数'?' 户':k==='车位数'?' 个':''}</dd>`).join('')}</dl><p class="note">来源：宁波银行上刊点位明细.xlsx · 道闸<br>距离沿用原表，非当前位置距离；收盘售价单位原表未注明。</p>`;$('#scroll').scrollTop=0;expanded=false;layout();updatePins();map.setView(d.coord,Math.max(map.getZoom(),16),{animate:false});updatePins();$('#checkin').onclick=()=>toggleCheckin(n);$('#copyAddress').onclick=async()=>{const address='上海市'+d['区域']+'区'+d['地址'];try{await navigator.clipboard.writeText(address);toast('地址已复制')}catch{const ta=document.createElement('textarea');ta.value=address;ta.style.cssText='position:fixed;top:0;left:-9999px';document.body.appendChild(ta);ta.select();const ok=document.execCommand('copy');ta.remove();toast(ok?'地址已复制':'无法自动复制，请长按详情中的地址复制')}}}
...
function renderLabels(){if(!map||!labelLayer)return;labelLayer.clearLayers();const z=map.getZoom(),used=[];const candidates=GEO.labels.filter(v=>map.getBounds().contains(v[1])).sort((a,b)=>{const rank=v=>v[2]==='park'?0:['primary','secondary'].includes(v[3])?1:v[3]==='tertiary'?2:3;return rank(a)-rank(b)});for(const [name,p,kind,road] of candidates){if(z<15&&kind==='road'&&!['primary','secondary','tertiary'].includes(road))continue;const pixel=map.latLngToContainerPoint(p);if(pixel.x<30||pixel.x>map.getSize().x-35||pixel.y<45||pixel.y>map.getSize().y-20)continue;const width=Math.max(55,name.length*11);if(used.some(q=>Math.abs(q.x-pixel.x)<(q.w+width)/2+14&&Math.abs(q.y-pixel.y)<32))continue;used.push({x:pixel.x,y:pixel.y,w:width});L.marker(p,{interactive:false,keyboard:false,pane:'labels',icon:L.divIcon({className:'street-name '+(kind==='park'?'park-name':''),html:escapeHTML(name),iconSize:[width,20],iconAnchor:[width/2,10]})}).addTo(labelLayer);if(used.length>65)break}}
function initMap(){map=L.map('map',{zoomControl:false,attributionControl:true,preferCanvas:true,minZoom:12,maxZoom:18,zoomSnap:.25,maxBounds:[[31.205,121.42],[31.335,121.585]],maxBoundsViscosity:.85});map.attributionControl.setPrefix('<a href="https://leafletjs.com" target="_blank" rel="noopener">Leaflet</a>');map.attributionControl.addAttribution('© <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">OpenStreetMap</a>');map.createPane('labels');map.getPane('labels').style.zIndex=450;map.getPane('labels').style.pointerEvents='none';labelLayer=L.layerGroup().addTo(map);markerGroup=L.layerGroup().addTo(map);leaderLayer=L.layerGroup().addTo(map);map.setView([31.264,121.502],14);
